...
async function insertNode(input: {
    ...
    const values = [
        ...
        input.is_exit_node,
        input.chain_id,
        input.hoprd_api_endpoint,
        input.hoprd_api_token,
        input.exit_node_pub_key,
        input.native_address,
    ];
...
async function insertNodes() {
    try {
        for (let i = 0; i < nodes.length; i++) {
            const addresses = await hoprAPI.getAddresses({
                apiEndpoint: nodes[i],
                apiToken: process.env.HOPRD_API_TOKEN as string,
            });
            await insertNode({
                id: addresses.hopr,
