...
export class MockFrameWallet extends MockWallet implements MockWallet {
    protected buildRequestBodies(): JsonRpcPayload[] {
        const requests = [];
        requests.push(getStandardPayload(JsonRpcMethod.NET_VERSION));
        requests.push(getStandardPayload(JsonRpcMethod.CHAIN_ID));
        requests.push(getStandardPayload(JsonRpcMethod.GET_BLOCK));
        requests.push(getStandardPayload(JsonRpcMethod.SYNCING));
