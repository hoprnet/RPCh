...
entry_node_peer_id=$(cat $DIR_SANDBOX/logs | grep "node1" -A 1 | grep "Peer Id" | awk '{ print $5 }')
exit_node_peer_id=$(cat $DIR_SANDBOX/logs | grep "node5" -A 1 | grep "Peer Id" | awk '{ print $5 }')
