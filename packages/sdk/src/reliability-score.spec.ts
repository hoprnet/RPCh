...
describe("test reliability score class", () => {
  ...
  function add20Metrics(peerId: string) {
    for (let count = 1; count <= 20; count++) {
      reliabilityScore.addMetric(peerId, `req${count}`, "success");
    }
  }
  ...
  it("should add metrics", () => {
    ...
    reliabilityScore.addMetric(ENTRY_NODE_PEER_ID, "request03", "none");
  ...
  it("should calculate score for ready nodes", () => {
    ...
    reliabilityScore.addMetric(ENTRY_NODE_PEER_ID, "req21", "none");
    reliabilityScore.addMetric(ENTRY_NODE_PEER_ID, "req22", "none");
