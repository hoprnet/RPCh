...
function createDefaultPackages(pgm) {
  const sql = [
    ...
    "(gen_random_uuid(), {name1}, {desc1}, {segs1}, now()),",
    "(gen_random_uuid(), {name2}, {desc2}, {segs2}, now()),",
    "(gen_random_uuid(), {name3}, {desc3}, {segs3}, now())",
  ].join(" ");
  const vals = {
    name1: "casual",
    desc1: "3 trades per week, 3 hours of DeFi browsing",
    segs1: 178e3,
    name2: "trader",
    desc2: "12 trades per week, 12 hours of DeFi browsing",
    segs2: 605.2e3,
    name3: "degen",
    desc3: "28 trades per week, 28 hours of DeFi browsing",
    segs3: 1424e3,
  };
  pgm.sql(sql, vals);
}
...
function createDefaultVouchers(pgm) {
  ...
  createVoucher(pgm, {
    ...
    code: "ETHROMEHACKERS",
    validUntil: new Date("2023-11-01"),
  });
}

function createVoucher(pgm, vals) {
  const sql = [
    "insert into vouchers (id, package_id, name, code, valid_until, created_at)",
    "values (gen_random_uuid(), (select id from packages where name = {packageName}), {name}, {code}, {validUntil}, now())",
...
function updateExistingVouchers(pgm) {
  const sql =
    "update vouchers set package_id = (select id from packages where name = {packageName});";
  pgm.sql(sql, { packageName: "degen" });
}
